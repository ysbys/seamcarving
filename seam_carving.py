...
import os
SEAM_COLOR = np.array([0, 0, 200])    # seam visualization color (BGR)
SEAM_COLOR2=np.array([255,255,255])
...
def visualize(im, boolmask=None, rotate=False):
    ...
    if boolmask is not None:
        ...
        vis[np.where(boolmask == False)] = SEAM_COLOR2
    if rotate:
        vis = rotate_image(vis, False)
    cv2.imshow("visualization", vis)
    cv2.waitKey(1)
    #保留图片
    #cv2.imwrite(os.getcwd()+"\\photo\\"+str(i)+".jpg",vis)
    #i=i+1
    return vis
...
def get_minimum_seam(im, mask=None, remove_mask=None):
    ...
    if remove_mask is not None:
        ...
        M[np.where(remove_mask > MASK_THRESHOLD_)] = -ENERGY_MASK_CONST * 100
...
def object_removal(im, rmask, mask=None, vis=False, horizontal_removal=False):
    ...
    while len(np.where(rmask > MASK_THRESHOLD_)[0]) > 0:
        ...
        if mask is not None:
            mask = remove_seam_grayscale(mask, boolmask)
    #不要拉伸去掉后面
    cv2.imwrite(os.getcwd()+"\\photo\\"+str(666)+".jpg",output)
...
def ysbys(dic):
    IM_PATH, MASK_PATH, OUTPUT_NAME, R_MASK_PATH = dic["im"], dic["mask"], dic["out"], dic["rmask"]
    ...
    USE_FORWARD_ENERGY = not dic["backward_energy"]
    ...
    if dic["resize"]:
        ...
        output = seam_carve(im, dy, dx, mask, dic["vis"])
        cv2.imwrite(OUTPUT_NAME, output)

    # object removal mode
    elif dic["remove"]:
        assert rmask is not None
        output = object_removal(im, rmask, mask, dic["vis"], dic["hremove"])
        cv2.imwrite(OUTPUT_NAME, output)
